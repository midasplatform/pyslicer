<?php
/*=========================================================================
 MIDAS Server
 Copyright (c) Kitware SAS. 26 rue Louis Guérin. 69100 Villeurbanne, FRANCE
 All rights reserved.
 More information http://www.kitware.com

 Licensed under the Apache License, Version 2.0 (the "License");
 you may not use this file except in compliance with the License.
 You may obtain a copy of the License at

         http://www.apache.org/licenses/LICENSE-2.0.txt

 Unless required by applicable law or agreed to in writing, software
 distributed under the License is distributed on an "AS IS" BASIS,
 WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 See the License for the specific language governing permissions and
 limitations under the License.
=========================================================================*/
define('MIDAS_PYSLICER_INVALID_POLICY', -151);
define('MIDAS_PYSLICER_INVALID_PARAMETER', -150);
define('MIDAS_PYSLICER_REMOTEPROCESSING_JOB_EXCEPTION', 3);

define('MIDAS_PYSLICER_SEGMENTATION_PIPELINE', 'segmentation');
define('MIDAS_PYSLICER_VOLUMERENDERING_PIPELINE', 'volumerendering');
define('MIDAS_PYSLICER_REGISTRATION_PIPELINE', 'registration');
 
define('MIDAS_PYSLICER_SEGMENTATION_INPUT_COUNT', 1);
define('MIDAS_PYSLICER_SEGMENTATION_OUTPUT_COUNT', 1);
define('MIDAS_PYSLICER_REGISTRATION_INPUT_COUNT', 1);
define('MIDAS_PYSLICER_REGISTRATION_OUTPUT_COUNT', 2);

define('MIDAS_PYSLICER_EXPECTED_INPUTS', 'expectedinputs');
define('MIDAS_PYSLICER_EXPECTED_OUTPUTS', 'expectedoutputs');

define('MIDAS_PYSLICER_INPUT_GENERATOR', 'inputgenerator');
define('MIDAS_PYSLICER_OUTPUT_GENERATOR', 'outputgenerator');

define('MIDAS_PYSLICER_REGISTRATION_OUTPUT_TRANSFORM_DESCRIPTION', 'output transform');
define('MIDAS_PYSLICER_REGISTRATION_OUTPUT_VOLUME_DESCRIPTION', 'output volume');

?>
