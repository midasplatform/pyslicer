...
class SlicerProcessJobManager():
    """This class should implement Pyslicer's own process manager. Pyslicer sitll uses Slicer's process manager as of now."""
...
class SlicerProcess(protocol.ProcessProtocol):
    """This class implements a twisted process which runs a python script within Slicer's Python environment."""
    ...
    def run(self):
        ...
        slicerCmdParts = [self.jobManager.slicerPath] + slicerArgs + [slicerPythonScript] + [str(self.jobId), self.jobManager.tmpDirRoot, jsonArgs]#['slicerjob']
